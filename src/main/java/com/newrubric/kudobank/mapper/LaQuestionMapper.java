...
public class LaQuestionMapper implements ResultSetMapper<LaQuestion>  {
	public LaQuestion map(int index, ResultSet rs, StatementContext ctx) throws SQLException {
		LaQuestion question = new LaQuestion();
		question.setLcsaName(rs.getString("lcsa_name"));
		question.setQuestionText(rs.getString("la_question"));
