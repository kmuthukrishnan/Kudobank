...
public class RCQuestionMapper implements ResultSetMapper<RCQuestion>  {
	public RCQuestion map(int index, ResultSet rs, StatementContext ctx) throws SQLException {
		RCQuestion question = new RCQuestion();
		question.setLcsaName(rs.getString("lcsa_name"));
		question.setQuestionText(rs.getString("rc_question"));
